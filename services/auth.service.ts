...
@Service({
  ...
  mixins: [
    authMixin(process.env.AUTH_API_KEY, {
      ...
      appHost: process.env.APP_HOST || 'https://gyvunai.biip.lt/app',
    }),
  ],
  hooks: {
    after: {
...
export default class AuthService extends moleculer.Service {
  ...
  async afterUserLoggedIn(ctx: any, data: any) {
    ...
    if (!user) {
      ...
      user = await ctx.call('users.update', {
        id: user.id,
        firstName: authUser.firstName,
        lastName: authUser.lastName,
        lastLogin: Date.now(),
    ...
    for (const authGroup of authGroups) {
      ...
      if (!tenant) {
        ...
        tenant = await ctx.call('tenants.update', {
          id: tenant.id,
          name: authGroup.name || tenant.name,
          code: authGroup.companyCode || tenant.code,
          email: authGroup.companyEmail || tenant.email,
          phone: authGroup.companyPhone || tenant.phone,
        });
      }
