exports.up = function (knex) {
  return knex.raw(`
      ...
      municipality_permits AS (
          SELECT 
              f.species_classifier_id,
              f.municipality ->> 'id' AS municipality_id,
              f.municipality ->> 'name' AS municipality_name,
              COUNT(DISTINCT f.permit_id) AS permits_count
          FROM filtered_permits f
          GROUP BY f.species_classifier_id, f.municipality ->> 'id', f.municipality ->> 'name'
      )
      SELECT 
          mp.species_classifier_id AS species_id,
          si.name AS species,
          si.name_latin AS species_lot,
          SUM(mp.permits_count) AS total_permits,
          JSONB_AGG(JSONB_BUILD_OBJECT(
              'id', mp.municipality_id,
              'name', mp.municipality_name,
              'permits', mp.permits_count
          )) AS municipalities
      FROM municipality_permits mp
      JOIN species_info si ON mp.species_classifier_id = si.species_classifier_id
      GROUP BY mp.species_classifier_id, si.name, si.name_latin;
    `);
};
